...
class DashboardScreen extends StatelessWidget {
  ...
  // ── SIMPLIFIED BANNER ──────────────────────────────────────────────────────
  //
  // Shows: logo · church name · tagline · city
  // Bottom-center: "View Profile" button → ChurchProfileScreen
  // Top-right: settings icon only (edit moved to profile page)
        ...
        child: SafeArea(
          ...
          child: Column(
            children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            profile.name.isNotEmpty ? profile.name : 'My Church',
                            style: TextStyle(
                                color: onPrimary,
                                fontSize: 20,
                                fontWeight: FontWeight.bold),
                          ...
                          if (profile.city.isNotEmpty) ...[
                            ...
                            Text(
                              [profile.city, if (profile.state.isNotEmpty) profile.state]
                                  .join(', '),
                              style: TextStyle(
              ...
              // ── View Profile button — centered at bottom of banner ────────
                ...
                child: Center(
                  child: TextButton.icon(
                    onPressed: () => Navigator.of(context).push(
                      MaterialPageRoute(
                          builder: (_) => const ChurchProfileScreen())),
                    icon: Icon(Icons.account_balance_outlined,
                        size: 15, color: onPrimary.withValues(alpha: 0.85)),
                    label: Text('View Church Profile',
                        style: TextStyle(
...
class _ChurchLogoState extends State<ChurchLogo> {
  ...
  void didUpdateWidget(ChurchLogo old) {
    ...
    if (old.logoPath != widget.logoPath) {
      if (old.logoPath.isNotEmpty) FileImage(File(old.logoPath)).evict();
      if (widget.logoPath.isNotEmpty) FileImage(File(widget.logoPath)).evict();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: logoPath.isNotEmpty && File(logoPath).existsSync()
          ? Image.file(File(logoPath), key: ValueKey(logoPath), fit: BoxFit.cover)
          : Icon(Icons.church, color: contrastOn(secondary), size: size * 0.55),
    );
  }
}

// Private alias used inside the banner
              ...
              child: Column(
                ...
                children: [
                  _SettingsTile(
                    ...
                    subtitle:  'View and edit name, logo, colors, contact info, and more',
                    onTap: () {
...
class _AppCard extends StatelessWidget {
  ...
  static IconData _appIconFor(String id) => _appIcons[id] ?? Icons.apps;

  @override
  Widget build(BuildContext context) {
    final icon    = _appIconFor(app.id);
